...
## ⚡ Antigravity 核心使命：分担实质编码与写单测，严禁只读审查浪费 Token (Workload Offloading & Token Saving Directive)
- **【核心定位与唯一宗旨：替 Codex 扛脏活累活，省 Codex Token】**：
  Antigravity 拥有全套文件读写与命令执行权限（`--dangerously-skip-permissions`）。它的价值是 **真正动手编写外围代码、写单测并跑通测试，分担 Codex 的编码工作量，节省 Codex 上下文 Token**，绝不是一个“文字评论家”！
- **🚫 绝对禁令（严禁只读清谈与子智能体递归转派）**：
  1. **严禁派发“只读审查（Read-only review）”或“规范符合性挑刺（SPEC COMPLIANCE review）”**；
  2. **严禁在提示词中附加 "Read-only: do not edit/modify"**，严禁让 Antigravity 产出数千字的点评报告倒灌回 Codex 撑爆上下文；
  3. **【严禁 Codex 子智能体调用 Antigravity】**：调度 Antigravity (`agy` CLI 或 MCP) 的权限**仅属于 Codex 主会话 (Master Codex / Root Agent)**。所有 Codex 派生子智能体（包括 `executor`, `code-reviewer`, `python-reviewer`, `architect`, `planner`, `analyst`, `debugger`, `explore` 等）**绝对禁止调用 `agy` 命令行或任何 Antigravity 工具**！子智能体必须自己动手读写代码与执行命令，坚决杜绝嵌套转包；
  4. 代码规范验证与代码审查由 Codex 亲自把握，或直接由本地 `pytest`/`ruff` 快速断言，绝不用 Antigravity 空耗 Token。
- **✅ 核心分担矩阵（必须是有实际代码落盘的硬核任务）**：
  1. 🔨 **外围与模板代码实际编写（动手落盘）**：
     - 数据抓取与清洗接入模块（如多源行情拉取、API 适配器实现）；
     - 繁琐的辅助工具类、序列化反序列化、防御性异常包裹、类型注解补充；
     - 配置文件、CI/CD GitHub Actions 脚本编写；
  2. 🧪 **批量单元测试编写与本地跑测（核心价值点）**：
     - 直接派发 Antigravity 编写/补齐 `tests/test_*.py`，并要求其在本地自动运行 `pytest` 确保通过；
     - Codex 零 Token 消耗直接收获全绿测试套件！
  3. 🗂️ **跨文件机械性重构与批量迁移**：
     - 涉及多文件的批量改名、废弃接口迁移、公共函数抽离。
- **【执行通道与极度紧凑输出契约】**：
  - 默认以 Tier 1 命令行直接调用：
    ```bash
    agy -p "<具体编码/测试编写任务与约束>" --dangerously-skip-permissions --print-timeout 10m
    ```
  - **返回契约约束**：提示词中必须要求 Antigravity **“直接落盘修改目标文件，仅在控制台简洁汇报修改的文件列表与测试结果（控制在 150 字以内），严禁长篇大论分析”**，把 Codex 的上下文 Token 节省到极致！
...
- **Default Subagent Routing (Antigravity Gateway)**:
  When spawning subagents for independent parallel subtasks, complex refactoring, code exploration, or multi-perspective code review, **DEFAULT TO CALLING THE ANTIGRAVITY MCP TOOLS (`ask_antigravity` / `antigravity_code_review`) OR DELEGATING TO ROLE `antigravity`**. Antigravity is the primary external subagent powerhouse for this workspace. Only fall back to spawning standard internal child agents when explicitly asked by the user or when Antigravity tools are unavailable.
...
<agent_catalog>
Key roles:
- `antigravity` — Google Antigravity expert subagent (default external gateway for complex refactoring, codebase exploration, and multi-perspective review)
