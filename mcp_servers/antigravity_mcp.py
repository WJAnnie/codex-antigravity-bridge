...
TIER2_MODEL = os.environ.get("ANTIGRAVITY_MODEL", "agentrouter/deepseek-v4-flash").strip()
...
def get_engine_name() -> str:
    ...
    if key and key.strip():
        if time.time() < _gemini_38_cooling_until:
            remain = max(1, int((_gemini_38_cooling_until - time.time()) / 60))
            return f"{TIER2_MODEL} (Tier2中继 | 3.8冷却余{remain}分)"
        return f"{TIER1_MODEL} (Tier1优先 -> DeepSeek -> 2.5兜底)"
    return f"{TIER2_MODEL} (本地中继)"
...
async def _execute_antigravity_core(
    ...
    tid_prefix = f" [ASYNC:{task_id}]" if task_id else ""
    global _gemini_38_cooling_until
    gemini_38_cooling = (time.time() < _gemini_38_cooling_until)

    # =========================================================================
    # Tier 1 (优先首选): gemini-3.8-flash (Google 原生前沿预览模型)
    # =========================================================================
    if api_key and not gemini_38_cooling:
        try:
            ...
            log_event(f"[FALLBACK]{tid_prefix} Tier 1 ({TIER1_MODEL}) 受阻 ({short_e})，自动降级至 Tier 2 ({TIER2_MODEL})...")
            if task_id:
                rec = _load_task_record(task_id)
                if rec:
                    rec["status_detail"] = f"Tier 1 受阻，已切至 Tier 2 ({TIER2_MODEL}) 运行"
                    _save_task_record(rec)
    elif gemini_38_cooling and api_key:
        remain = max(1, int((_gemini_38_cooling_until - time.time()) / 60))
        log_event(f"[DISPATCH]{tid_prefix} Tier 1 ({TIER1_MODEL}) 配额冷却中 (余 {remain} 分钟)，优先启用 Tier 2 ({TIER2_MODEL})...")

    # =========================================================================
    # Tier 2 (次选备用): agentrouter/deepseek-v4-flash (本地中继稳健引擎)
    # =========================================================================
    tier2_err = None
    try:
        return await _try_agent_loop(_build_openai_cfg(TIER2_MODEL), is_gemini=False, tier_name=f"Tier 2 ({TIER2_MODEL})")
    except Exception as e2:
        tier2_err = e2
        short_e2 = str(e2).replace("\n", " ").strip()[:100]
        log_event(f"[FALLBACK]{tid_prefix} Tier 2 ({TIER2_MODEL}) 遇到异常 ({short_e2})，自动降级至 Tier 3 ({TIER3_MODEL}) 终极高额度兜底...")
        if task_id:
            rec = _load_task_record(task_id)
            if rec:
                rec["status_detail"] = f"Tier 2 异常，已切至 Tier 3 ({TIER3_MODEL}) 终极兜底"
                _save_task_record(rec)

    # =========================================================================
    # Tier 3 (终极兜底): gemini-2.5-flash (Google 原生 1500次/天高配额基准模型)
    # =========================================================================
    if api_key:
        ...
        raise RuntimeError(f"全链路推理梯队均不可用，最后异常: {tier2_err}")
